...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initD3Circles() {
        ...
        // Community-only: background image reduced to 70% width, right-justified
        const pictureWidth = Math.round(width * 0.7);
        const pictureX = Math.max(-1, width - pictureWidth - 1);
        const bgImage = svg.append('image')
            .attr('x', pictureX)
            .attr('y', -1)
            .attr('width', pictureWidth + 2)
            .attr('height', height + 2)
            .attr('preserveAspectRatio', 'xMidYMid slice')
            .attr('href', '../animations/test_balloon_background_pic.png');

        const defs = svg.append('defs');
        const revealMask = defs.append('mask')
            .attr('id', 'balloonRevealMask')
            .attr('maskUnits', 'userSpaceOnUse');
        revealMask.append('rect')
            .attr('x', -1)
            .attr('y', -1)
            .attr('width', width + 2)
            .attr('height', height + 2)
            .attr('fill', 'white');
        const maskRoot = revealMask.append('g');

        // Edge fade mask for the image to softly reveal from its left boundary
        const edgeFadeGradient = defs.append('linearGradient')
            .attr('id', 'edgeFadeGradient')
            .attr('gradientUnits', 'userSpaceOnUse')
            .attr('x1', pictureX)
            .attr('y1', 0)
            .attr('x2', pictureX + 110)
            .attr('y2', 0);
        edgeFadeGradient.append('stop').attr('offset', '0%').attr('stop-color', 'black');
        edgeFadeGradient.append('stop').attr('offset', '100%').attr('stop-color', 'white');

        const edgeFadeMask = defs.append('mask')
            .attr('id', 'edgeFadeMask')
            .attr('maskUnits', 'userSpaceOnUse');
        // Left fade strip (110px)
        edgeFadeMask.append('rect')
            .attr('x', pictureX)
            .attr('y', -1)
            .attr('width', 110)
            .attr('height', height + 2)
            .attr('fill', 'url(#edgeFadeGradient)');
        // Middle visible area (white)
        edgeFadeMask.append('rect')
            .attr('x', pictureX + 110)
            .attr('y', -1)
            .attr('width', Math.max(0, pictureWidth - 220))
            .attr('height', height + 2)
            .attr('fill', 'white');
        // Right fade: stronger and wider (140px)
        const rightStart = pictureX + Math.max(0, pictureWidth - 140);
        const rightFadeGradient = defs.append('linearGradient')
            .attr('id', 'edgeFadeRightGradient')
            .attr('gradientUnits', 'userSpaceOnUse')
            .attr('x1', rightStart)
            .attr('y1', 0)
            .attr('x2', pictureX + pictureWidth)
            .attr('y2', 0);
        rightFadeGradient.append('stop').attr('offset', '0%').attr('stop-color', 'white');
        rightFadeGradient.append('stop').attr('offset', '100%').attr('stop-color', 'black');
        edgeFadeMask.append('rect')
            .attr('x', rightStart)
            .attr('y', -1)
            .attr('width', 140)
        ...
        const clipRoot = imageClip.append('g');
        bgImage.attr('mask', 'url(#edgeFadeMask)');
        ...
        svg.append('rect')
            ...
            .attr('fill', '#ffffff')
            .attr('opacity', 1)
            .attr('mask', 'url(#balloonRevealMask)');
        ...
        // Create mirror groups inside the mask so holes track balloon positions (Community-only)
        const maskGroups = maskRoot.selectAll('g')
            .data(nodes)
            .enter()
            .append('g')
            .attr('transform', d => `translate(${d.x}, ${d.y})`);

        const clipGroups = clipRoot.selectAll('g')
            .data(nodes)
            .enter()
            .append('g')
            .attr('transform', d => `translate(${d.x}, ${d.y})`);
        ...
        balloonGroups.each(function(d){
            ...
            for (let layer = 0; layer < d.layers; layer++) {
                ...
                // Community-only: increase opacity by ~30% (make colours more visible)
                const layerOpacity = Math.max(0.05, Math.min(1, (0.4 - (layer * 0.05)) * 0.5 * 1.3));
        ...
        maskGroups.each(function(d){
            ...
            for (let layer = 0; layer < d.layers; layer++) {
                ...
                const curvedGrowth = Math.pow(normalizedLayer, 1.75);
                const layerSize = Math.max(1, d.baseSize * (0.3 + curvedGrowth * 0.5));
                const maskAlpha = 1.0; // cut full holes so image shows clearly under balloons
                const distanceFromEdge = Math.min(layer, maxLayer - layer);
                const constructionDelay = distanceFromEdge * 150;
                const deconstructionDelay = layer * 150;
                group.append('circle')
        ...
        // Parallel white circles in clipPath (use circles that the image will clip to)
        clipGroups.each(function(d){
            ...
            for (let layer = 0; layer < d.layers; layer++) {
                ...
                const constructionDelay = distanceFromEdge * 150;
                const deconstructionDelay = layer * 150;
                // In clipPath, opacity is ignored; build circles and animate radius via transitions for timing parity
                const circle = group.append('circle')
                    .attr('r', 0)
                    .attr('fill', '#ffffff')
                    .datum({ layer, layerSize, constructionDelay, deconstructionDelay, isConstructed:false });
                circle.transition()
                    .delay(constructionDelay)
                    .duration(720)
                    .attr('r', layerSize)
        ...
        function animate(){
            ...
            balloonGroups.each(function(d, i){
                const group = d3.select(this);
                const mgroup = d3.select(maskGroups.nodes()[i]);
                const cgroup = d3.select(clipGroups.nodes()[i]);
                ...
                if ((d.x + (d.maxRadius||0)) >= (RIGHT_BOUNDARY - FADE_LEAD) && !d.isDeconstructing) {
                    ...
                    mgroup.selectAll('circle').transition().delay(ld=>ld.deconstructionDelay).duration(600).attr('opacity',0).on('end', ld=>{ ld.isConstructed=false; });
                    cgroup.selectAll('circle').transition().delay(ld=>ld.deconstructionDelay).duration(600).attr('r', 0).on('end', ld=>{ ld.isConstructed=false; });
                }
            if (d.x > width + 200) {
                ...
                if (shouldConstruct) {
                    ...
                    group.selectAll('circle').each(function(ld, li){ d3.select(this).transition().delay(ld.constructionDelay||0).duration(720).attr('opacity', Math.max(0.05, ld.layerOpacity||0.3)).on('end', ()=>{ ld.isConstructed=true; }); });
                    mgroup.selectAll('circle').each(function(ld){ d3.select(this).transition().delay(ld.constructionDelay||0).duration(720).attr('opacity', ld.maskAlpha).on('end', ()=>{ ld.isConstructed=true; }); });
                    cgroup.selectAll('circle').each(function(ld){ d3.select(this).transition().delay(ld.constructionDelay||0).duration(720).attr('r', ld.layerSize || 20).on('end', ()=>{ ld.isConstructed=true; }); });
                }
                group.attr('transform', `translate(${d.x}, ${currentY})`);
                mgroup.attr('transform', `translate(${d.x}, ${currentY})`);
                cgroup.attr('transform', `translate(${d.x}, ${currentY})`);
                group.selectAll('circle').each(function(ld){
                    const time = carouselTime * d.animationSpeed + d.animationOffset;
                    const scale = 1 + Math.sin(time) * 0.3;
                    const opacity = Math.min(1, ld.layerOpacity * 1.0 * (0.8 + Math.sin(time * 0.7) * 0.2));
                ...
                // Keep mask radii in sync for visual parity
                mgroup.selectAll('circle').each(function(ld){
                    const time = carouselTime * d.animationSpeed + d.animationOffset;
                    const scale = 1 + Math.sin(time) * 0.3;
                    const newRadius = (ld.layerSize || 20) * scale;
                    if (!isNaN(newRadius) && newRadius > 0) {
                        d3.select(this).attr('r', newRadius);
                    }
                });
                cgroup.selectAll('circle').each(function(ld){
                    const time = carouselTime * d.animationSpeed + d.animationOffset;
                    const scale = 1 + Math.sin(time) * 0.3;
                    const newRadius = (ld.layerSize || 20) * scale;
                    if (!isNaN(newRadius) && newRadius > 0) {
